...
(
cd src/build || exit 1
cmake .. -DCMAKE_C_COMPILER="$_TARGET-cc" -DCMAKE_SYSTEM_NAME=Darwin -DCMAKE_INSTALL_PREFIX=/var/usr -DCMAKE_FIND_ROOT_PATH_MODE_LIBRARY=ONLY -DCMAKE_FIND_ROOT_PATH_MODE_INCLUDE=ONLY -DCMAKE_FIND_ROOT_PATH="$_SDK/var/usr"
"$_MAKE" -j8
...
for bin in bin/*; do
    lib="$("$_TARGET-otool" -L "$bin" | grep libzip | awk '{print $1}')"
    if [ -n "$lib" ]; then
